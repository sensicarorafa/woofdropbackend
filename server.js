...
async function handleUserRegistration(ctx, referralCode) {
    const telegramId = ctx.from.id;
    let existingUser = await User.findOne({ 'user.id': telegramId });

    // Check if user was referred and handle referral points if they are a new user
    if (referralCode && !existingUser) {
        await addReferralPoints(referralCode);
    }

    // If user is already registered and has a referral code
    if (referralCode && existingUser) {
        try {
            await ctx.reply(`You have already been referred previously`);
        } catch (error) {
    ...
    // Register new user if not found in the database
    if (!existingUser) {
        let uniqueReferralCode;
        let isUnique = false;
        ...
        // Create new user with relevant details
        const newUser = new User({
            pointsNo: 0,
            referralPoints: 0,
            user: {
...
// Function to send welcome message with photo and links
...
bot.start(async (ctx) => {
    try {
        const referralCode = ctx.message.text.split(' ')[1]; // Extract payload for /start
        await handleUserRegistration(ctx, referralCode);
...
// Startapp command handler
bot.command('startapp', async (ctx) => {
    try {
        const referralCode = ctx.message.text.split(' ')[1]; // Extract payload for /startapp
        await handleUserRegistration(ctx, referralCode);
        await sendWelcomeMessage(ctx);
    } catch (error) {
        console.log(error);
